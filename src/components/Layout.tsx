...
const adminGroups: NavGroup[] = [
  ...
  { label: 'Snack Operations', items: [
    { to: '/snack', label: 'Snack Overview' },
    { to: '/members', label: 'Members' },
    { to: '/menu', label: 'Food Menu' },
    { to: '/wallet', label: 'Advance Wallet' },
    { to: '/orders', label: 'Daily Orders' },
...
export function Layout() {
  ...
  const activeGroup = useMemo(
    () => groups.find(group => group.items.some(item => location.pathname === item.to || location.pathname.startsWith(`${item.to}/`)))?.label || groups[0]?.label,
    [groups, location.pathname],
  )
  const [openSector, setOpenSector] = useState(activeGroup)

  useEffect(() => {
    setMobileOpen(false)
    setOpenSector(activeGroup)
  ...
  return <div className="shell">
    <aside className={`sidebar${mobileOpen ? ' mobile-open' : ''}`}>
      <div className="sidebar-top">
        ...
        <button type="button" className="mobile-menu-button" aria-expanded={mobileOpen} onClick={() => setMobileOpen(open => !open)}>
      ...
      <div className="sidebar-panel" id="mobile-navigation">
        <nav className="sector-nav">
          {groups.map(group => {
            const expanded = openSector === group.label
            return <section className={`nav-sector${expanded ? ' expanded' : ''}`} key={group.label}>
              <button type="button" className="sector-toggle" aria-expanded={expanded} onClick={() => setOpenSector(expanded ? '' : group.label)}>
                <span>{group.label}</span><b aria-hidden="true">{expanded ? '−' : '+'}</b>
              </button>
              <div className="sector-links">
                {group.items.map(item => <NavLink key={item.to} to={item.to} end>{item.label}</NavLink>)}
              </div>
            </section>
          })}
        </nav>
