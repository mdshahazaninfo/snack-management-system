...
const periodStartDate=(period:'today'|'week'|'month')=>{
  if(period==='today')return today()
  const now=new Date(`${today()}T12:00:00+06:00`)
  if(period==='week')now.setUTCDate(now.getUTCDate()-6)
...
export function SnackOverviewPage(){
  ...
  useEffect(()=>{void(async()=>{
    setError(null)
    const from=businessDayStartIso(periodStartDate(period))
    const [{data:o,error:oe},{data:b,error:be},{data:i,error:ie}]=await Promise.all([
    ...
    const first=[oe,be,ie].find(Boolean)
  ...
  return <div className="premium-page"><ErrorText error={error}/>
    ...
    <div className="bento-grid snack-bento">
      ...
      <section className="bento-card circular-card"><div className="card-title"><div><small>PERFORMANCE</small><h2>Margin pulse</h2></div></div><div className="ring" style={{'--value':`${Math.max(0,Math.min(100,sales?margin/sales*100:0))}%`} as CSSProperties}><div><strong>{sales?`${(margin/sales*100).toFixed(0)}%`:'0%'}</strong><span>margin</span></div></div><p>{money(margin)} retained after estimated item cost.</p></section>
