...
type Order = { id:string; invoice_no:string; total:number; status:string; created_at:string; balance_after?:number; email_status?:string; email_error?:string|null; receipt_email?:string|null; members:{full_name:string}|null }
...
export function OrdersPage(){
 ...
 const[error,setError]=useState<string|null>(null),[success,setSuccess]=useState<string|null>(null),[receipt,setReceipt]=useState<Receipt|null>(null),[sending,setSending]=useState<string|null>(null),[posting,setPosting]=useState(false)
 ...
 const load=async()=>{const[{data:m,error:me},{data:i,error:ie},{data:o,error:oe}]=await Promise.all([
  ...
  supabase.from('orders').select('id,invoice_no,total,status,created_at,balance_after,email_status,email_error,receipt_email,members(full_name)').order('created_at',{ascending:false}).limit(20),
 ...
 const post=async()=>{
  ...
  postingLock.current=true;setPosting(true)
  const selected=members.find(m=>m.id===memberId),cartSnapshot=cart.map(x=>({...x}))
  try{
   const idempotencyKey=crypto.randomUUID()
   const{data,error:postError}=await supabase.rpc('create_order',{p_member_id:memberId,p_items:cart.map(x=>({menu_item_id:x.id,quantity:x.qty})),p_idempotency_key:idempotencyKey})
   if(postError){setError(postError.message);return}
   const order=Array.isArray(data)?data[0]:data
   if(!order||!selected){setError('The order was not returned by the server. Check Recent Orders before retrying.');await load();return}
   setCart([])
   const remaining=order.balance_after==null?Number(selected.balance)-Number(order.total):Number(order.balance_after)
   const receiptOrder={...order,members:{full_name:selected.full_name},employee_id:selected.employee_id,remaining_balance:remaining,items:cartSnapshot} as Receipt
   ...
   postingLock.current=false;setPosting(false)
  }
 }
 ...
 return <>
  ...
  <div className="grid-2 orders-grid"><Card title="Build order"><label>Member<select value={memberId} onChange={e=>setMemberId(e.target.value)} disabled={posting}><option value="">Select member</option>{members.map(m=><option key={m.id} value={m.id}>{m.employee_id} — {m.full_name} ({money(m.balance)}){m.email&&m.email_receipt_enabled?' · Email ready':''}</option>)}</select></label><label>Search by name / SKU<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Scan or type…" disabled={posting}/></label><div className="item-grid">{shown.map(i=><button className="item-btn" key={i.id} onClick={()=>add(i)} disabled={posting}><b>{i.name}</b><small>{i.sku} · {money(i.selling_price)}</small></button>)}</div></Card>
  <Card title="Cart">{cart.length?<>{cart.map(x=><div className="cart-row" key={x.id}><div><b>{x.name}</b><small>{x.qty} × {money(x.selling_price)}</small></div><div><button className="tiny" disabled={posting} onClick={()=>setCart(c=>c.map(y=>y.id===x.id?{...y,qty:Math.max(1,y.qty-1)}:y))}>−</button><button className="tiny" disabled={posting} onClick={()=>setCart(c=>c.map(y=>y.id===x.id?{...y,qty:y.qty+1}:y))}>+</button><button className="tiny danger-btn" disabled={posting} onClick={()=>setCart(c=>c.filter(y=>y.id!==x.id))}>×</button></div></div>)}<div className="cart-total"><span>Total</span><strong>{money(total)}</strong></div><button onClick={post} disabled={posting}>{posting?'Posting order…':'Post order & email bill'}</button></>:<Empty text="Add menu items to begin."/>}</Card></div>
  ...
  <Card title="Recent orders">{orders.length?<div className="table-wrap"><table><thead><tr><th>Invoice</th><th>Member</th><th>Time</th><th>Total</th><th>Status</th><th>Email bill</th><th/></tr></thead><tbody>{orders.map(o=><tr key={o.id}><td>{o.invoice_no}</td><td>{o.members?.full_name}</td><td>{new Date(o.created_at).toLocaleString('en-BD',{timeZone:'Asia/Dhaka'})}</td><td>{money(o.total)}</td><td>{o.status}</td><td><span className={`pill email-${o.email_status||'not_requested'}`} title={o.email_error||''}>{emailLabel(o.email_status)}</span>{o.receipt_email&&<small className="table-subtext">{o.receipt_email}</small>}</td><td><div className="row-actions">{o.status==='posted'&&<button className="tiny secondary" disabled={sending===o.id} onClick={()=>void sendReceipt(o.id,o.invoice_no)}>{sending===o.id?'…':'Email'}</button>}{isAdmin&&o.status==='posted'&&<button className="tiny danger-btn" onClick={()=>void voidOrder(o.id)}>Void</button>}</div></td></tr>)}</tbody></table></div>:<Empty/>}</Card>
 </>
}
