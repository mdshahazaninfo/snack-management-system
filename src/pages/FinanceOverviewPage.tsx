...
export function FinanceOverviewPage(){
 const [tx,setTx]=useState<Tx[]>([]),[accounts,setAccounts]=useState<Account[]>([]),[budgets,setBudgets]=useState<Budget[]>([]),[recurring,setRecurring]=useState<Recurring[]>([]),[goals,setGoals]=useState<Goal[]>([])
 useEffect(()=>{(async()=>{const[a,t,b,r,g]=await Promise.all([
 ...
 ]);setAccounts((a.data as Account[])||[]);setTx((t.data as unknown as Tx[])||[]);setBudgets((b.data as Budget[])||[]);setRecurring((r.data as Recurring[])||[]);setGoals((g.data as Goal[])||[])})()},[])
 ...
 <section className="bento-card"><div className="card-title"><div><small>ACCOUNTS</small><h2>Balance summary</h2></div></div><div className="account-mini-list">{accounts.slice(0,5).map(a=><div key={a.name}><span><b>{a.name}</b><small>{a.account_type}</small></span><strong>{money(a.balance)}</strong></div>)}</div></section>
