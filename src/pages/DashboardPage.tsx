...
export function DashboardPage(){
 ...
 <div className="hero-metrics dashboard-summary-cards">
  ...
  <Link to="/snack" className="dashboard-summary-card sales-summary"><span className="summary-icon">৳</span><small>Snack sales</small><strong>{money(sales)}</strong><em>{money(margin)} margin</em></Link>
  <Link to="/personal-finance" className="dashboard-summary-card balance-summary"><span className="summary-icon">◉</span><small>Personal balance</small><strong>{money(totalBalance)}</strong><em>{accounts.length} {accounts.length===1?'account':'accounts'}</em></Link>
 </div>
